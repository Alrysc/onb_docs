...
* Access to many functions related to Custom Gauge
    * `Battle.get_cust_gauge_value`, which returns a double with value `0.0` at mpty gauge and `1.0` or higher at full, as a percentage completion of the gauge
...
* HitProps builder pattern allows custom durations for Stun, Freeze, Blind, and Confuse
